...
window.ContextHubJQ=jQuery.noConflict(true);
(function(a){if(!Function.prototype.bind){Function.prototype.bind=function(b){if(typeof this!=="function"){throw new TypeError("Function.prototype.bind - what is trying to be bound is not callable")
}var f=Array.prototype.slice.call(arguments,1),e=this,c=function(){},d=function(){return e.apply(this instanceof c&&b?this:b,f.concat(Array.prototype.slice.call(arguments)))
};
c.prototype=this.prototype;
d.prototype=new c();
return d
}
}window.ContextHub=a.extend(window.ContextHub,{Constants:{EVENT_NAMESPACE:"contexthub",EVENT_INITIALIZED:"initialized",EVENT_REGISTER:"register",EVENT_STORE_READY:"ready",EVENT_DATA_UPDATE:"data",EVENT_CONFIG_LOADED:"config-loaded",PERSISTENCE_CONTAINER_NAME:"ContextHubPersistence",SERVICE_RAW_RESPONSE_KEY:"/_/raw-response",SERVICE_RESPONSE_TIME_KEY:"/_/response-time",SERVICE_LAST_URL_KEY:"/_/url",STORE_CONFIGURATION_KEY:"/_/store-configuration",CONFIGURATION_CHECKSUM_KEY:"_/store-configuration/checksum",CONTAINER_VISIBLE:"/_/container-visible"}});
ContextHub.console=function(b){if(ContextHubKernelConfig.debug&&window.console&&typeof(window.console[b])==="function"){window.console[b].apply(window.console,Array.prototype.slice.call(arguments,1))
}};
ContextHub.console("time","contexthub.js");
ContextHub.console("timeStamp","contexthub.start")
}(ContextHubJQ));
ContextHub.console("log","[+] loading [contexthub.shared] ContextHub.Shared.js");
(function(e,c,b,g){var f=function(i,h){h=h||2;
i=String(i);
var j=h-i.length;
if(j>0){f=Math.pow(10,Math.min(j,20));
i=String(f).slice(1)+i
}return i
};
var a={};
var d={start:function(h){a[h]=+new Date;
return a[h]
},finish:function(i){var h=a[i];
return h?(+new Date-h):0
}};
ContextHub.Shared={pad:f,timers:d}
}(ContextHubJQ,this,document));
ContextHub.console("log","[+] loading [contexthub.utils] ContextHub.Utils.cookie.js");
(function(h,j,k,d){j.ContextHub=h.extend({Utils:{}},j.ContextHub);
var f={path:"/",expires:d,domain:d,secure:false,trimUndefined:true};
var i=function(t,v,r){if(typeof(t)==="undefined"){return false
}r=h.extend({},f,r);
if(r.trimUndefined){var u=h.type(v);
v=(u==="undefined"||u==="null")?"":v
}if(r.expires){switch(h.type(r.expires)){case"date":break;
case"number":var q=new Date();
q.setDate(q.getDate()+r.expires);
r.expires=q;
break;
default:r.expires=d
}}r.secure=r.secure===true;
var s=[encodeURIComponent(t),"=",encodeURIComponent(v),r.expires?("; expires="+r.expires.toUTCString()):"",r.domain?("; domain="+r.domain):"",r.path?("; path="+r.path):"",r.secure?"; secure":""].join("");
k.cookie=s;
return s
};
var p=function(r){var s=g();
var q=null;
h(s).each(function(t,v){var w=a(v);
var u=c(v);
if(u===r){q=w
}return q===null
});
...
h(o(q)).each(function(s,t){var v=p(t);
var u=h.type(v);
if(u!=="undefined"&&u!=="null"){r[t]=v
}});
return r
};
var m=function(t,r){var q=h.type(r);
if(q==="object"&&h.isArray(data)){q="array"
}switch(q){case"regexp":return r.test(t);
case"string":return t===r;
case"function":return r(t)===true;
case"array":var s=false;
h(r).each(function(u,v){s=m(t,v);
return !s
});
return s;
default:return false
}};
var g=function(){return k.cookie?k.cookie.split(";"):[]
};
var c=function(q){return decodeURIComponent(h.trim(q.split("=")[0]))
};
var a=function(q){return decodeURIComponent(h.trim(q.split("=").slice(1).join("=")))
};
var o=function(q){var t=typeof(q)==="undefined";
var s=g();
var r=[];
h(s).each(function(u,v){v=c(v);
if(v.length&&(h.inArray(v,r)===-1)&&(t||m(v,q))){r.push(v)
}});
return r.sort()
};
var l=function(q){return p(q)!==null
};
var n=function(r,q){i(r,"",h.extend({},q,{expires:-1}))
};
var e=function(r,q){h(o(r)).each(function(s,t){n(t,q)
})
};
ContextHub.Utils.Cookie={setItem:i,getItem:p,getAllItems:b,getKeys:o,exists:l,removeItem:n,vanish:e}
}(ContextHubJQ,this,document));
ContextHub.console("log","[+] loading [contexthub.utils] ContextHub.Utils.json.js");
(function($,window,document,undefined){window.ContextHub=$.extend({Utils:{}},window.ContextHub);
var nativeJSONSupported=JSON&&JSON.stringify&&JSON.parse;
...
ContextHub.Utils.JSON={stringify:nativeJSONSupported?JSON.stringify:stringify,parse:function(data){var json;
try{json=(nativeJSONSupported?JSON.parse:parse)(data)
}catch(e){json={}
}return json
}}
}(ContextHubJQ,this,document));
(function(c,g,h,b){g.ContextHub=c.extend({Utils:{}},g.ContextHub);
var e=function(l){l=(c.type(l)!=="string")?"":c.trim(l);
l=c.trim(l.replace(/\/\/*/g,"/").replace(/(^\/|\/$)/g,""));
if(l.length){l=c.grep(c.map(l.split("/"),c.trim),function(m){return m.length
})
...
var i=function(l,m){m=e(m);
l=c.extend(true,{},l);
if(m){var n=l;
c.each(m.slice(0,-1),function(o,p){n=n[p];
return c.type(n)==="object"
});
if(n){delete (n[m.slice(-1)]);
l=a(l)
}}return l
};
var j=function(m,o,l){o=o||"/";
var p=[];
var n=c.type(m);
if(n==="object"||n==="array"){c.each(m,function(r,s){var q=o+r;
p.push(q);
n=c.type(s);
if(n==="object"||n==="array"){c.merge(p,j(s,q+"/"))
}})
}return p.sort(l)
};
var a=function(l){l=c.extend(true,{},l);
var m=j(l,null,function(o,n){var p=o.split("/").length;
var q=n.split("/").length;
if(p!=q){return(p>q)?-1:1
}return(o==n)?0:((o>n)?1:-1)
});
c.each(m,function(n,o){var p=k(l,o);
if(c.type(p)==="object"&&c.isEmptyObject(p)){l=i(l,o)
}});
return l
};
var d=function(l,m){return c.extend(true,{},l,m)
};
c.extend(ContextHub.Utils.JSON,{tree:{sanitizeKey:e,setItem:f,getItem:k,removeItem:i,getKeys:j,cleanup:a,addAllItems:d}})
}(ContextHubJQ,this,document));
ContextHub.console("log","[+] loading [contexthub.utils] ContextHub.Utils.eventing.js");
(function(h,n,u,k){n.ContextHub=h.extend({Utils:{}},n.ContextHub);
var w=(function(){var E=h.grep([n.requestAnimationFrame,n.msRequestAnimationFrame,n.mozRequestAnimationFrame,n.webkitRequestAnimationFrame,n.oRequestAnimationFrame],h.isFunction).shift();
if(!E){var D=0;
E=function(J,F){var G=+new Date;
...
var A=(function(){var D=h.grep([n.cancelAnimationFrame,n.cancelRequestAnimationFrame,n.msCancelRequestAnimationFrame,n.mozCancelRequestAnimationFrame,n.webkitCancelRequestAnimationFrame,n.oCancelRequestAnimationFrame],h.isFunction).shift();
if(!D){D=function(E){clearTimeout(E)
}
}return D
})();
var g={};
var r=function(E,F,D){E=h.trim(E);
if(E.length){D=h.extend({},{defer:this.config.defer},D);
m.call(this,h.trim(this.config.namespace),y(E,F,D))
}};
var m=function(F,H){var E="/"+F+"/"+H.channel;
var G=ContextHub.Utils.JSON.tree.getItem(this.queue,E)||{};
G=h.extend({},{executeAt:null,data:[]},H._,G);
var D=+new Date+H.defer;
...
var o=function(E){var D=h.extend({},E,{data:[]});
var F={};
h.each(E.data,function(G,H){F[H.key]=H;
F[H.key]._idx=G
});
F=h.map(F,function(G){return G
}).sort(function(H,G){return H._idx-G._idx
});
h.map(F,function(G){delete (G._idx);
return G
});
D.data=F;
return D
};
var t=function(D){if(D.indexOf(this.config.namespace+"-")!==0){D=this.config.namespace+"-"+D
}return g[D]===true
};
var C=function(D,H){var F=this.config.namespace+"-"+D;
H=o(H);
H.event=F;
var J=H.duration?"("+H.duration+") ":"";
ContextHub.console("log",e(),F,J+"-",H);
for(var G=F.split(":"),E=G.length;
E>0;
E--){var I=G.slice(0,E).join(":");
h(this.config.broadcast).trigger(I,H);
g[I]=true
}H=null;
delete ((this.queue[this.config.namespace]||{})[D]);
...
}if((this.eventingCounter++%this.periodicity)===0){h.each(this.queue[this.config.namespace]||{},function(D,E){if(!E.paused&&+new Date>E.executeAt){C.call(this,D,E)
}}.bind(this))
...
var c=function(){return this.running
};
var b=function(){this.queue={}
};
var z=function(E,D,F){this.queue=ContextHub.Utils.JSON.tree.setItem(this.queue,"/"+this.config.namespace+"/"+E+"/"+D,F)
};
var x=function(E,D){return ContextHub.Utils.JSON.tree.getItem(this.queue,"/"+this.config.namespace+"/"+E+"/"+D)
};
var v=function(D){z.call(this,D,"executeAt",0)
};
var j=function(D){z.call(this,D,"paused",true)
};
var d=function(D){z.call(this,D,"paused",k)
};
var l=function(D){return x.call(this,D,"paused")==true
};
var q=function(F,G,E){F=F.split(" ");
E=E?("."+E):"";
for(var D=0;
D<F.length;
D++){var H=F[D];
if(H.indexOf(G+"-")!==0){F[D]=G+"-"+H+E
}}return F.join(" ")
};
var p=function(G,I,F,E){G=q(G,this.config.namespace,F);
...
var B=function(E,D){E=q(E,this.config.namespace,D);
h(this.config.broadcast).off(E)
};
function y(E,F,D){return{channel:E,data:F,defer:D.defer,_:D._||{}}
...
}return{log:this.log,trigger:r.bind(this),getQueue:f.bind(this),isRunning:c.bind(this),enableEventing:i.bind(this),disableEventing:a.bind(this),alreadyTriggered:t.bind(this),clearQueue:b.bind(this),flush:v.bind(this),pause:j.bind(this),resume:d.bind(this),isPaused:l.bind(this),on:p.bind(this),off:B.bind(this),namespace:this.config.namespace,broadcast:this.config.broadcast}
};
...
ContextHub.console("log","[+] loading [contexthub.utils] ContextHub.Utils.persistence.js");
(function(e,f,h,b){f.ContextHub=e.extend({Utils:{}},f.ContextHub);
function g(k,j,l){return{name:k,isSupported:l?l:function(){return true
},getInterface:function(n){var m=j(n);
return{name:k,getItem:m.getItem,setItem:m.setItem,removeItem:m.removeItem,getKeys:m.getKeys,getTree:m.getTree}
}}
}g.prototype.setItem=function(l,k,j,m){l(ContextHub.Utils.JSON.tree.setItem(k(),j,m));
return this
};
g.prototype.getItem=function(k,j){return ContextHub.Utils.JSON.tree.getItem(k(),j)
};
g.prototype.removeItem=function(l,k,j){l(ContextHub.Utils.JSON.tree.removeItem(k(),j));
return this
};
var c=new g("null",function(){return{setItem:function(){return this
},getItem:function(){return{}
},removeItem:function(){},getKeys:function(){return[]
},getTree:function(){return{}
}}
});
var i=function(){var j=false;
e(e.merge([this.config.mode],this.config.fallback||[])).each(function(k,l){if(l&&l.isSupported()){j=true;
this.config.mode=l
}return !j
}.bind(this));
if(!j){this.config.mode=c
}return j
};
ContextHub.Utils.Persistence=function(k){this.config=e.extend({},ContextHub.Utils.Persistence.defaultConfig,k);
var l=this.config.mode;
var m=i.call(this);
var j={initialized:m,usingFallback:(this.config.mode!==l),window:this.config.window,container:this.config.container};
e.extend(j,this.config.mode.getInterface(this));
return j
};
ContextHub.Utils.Persistence.prototype.PersistenceMode=g;
ContextHub.Utils.Persistence.Modes={LOCAL:new g("local",function d(p){var m=p.config;
var k=m.container;
var q=m.window.localStorage;
var o=function(){var r=ContextHub.Utils.JSON.parse(q.getItem(k));
return(e.type(r)==="object")?r:{}
};
var n=function(r){q.setItem(k,ContextHub.Utils.JSON.stringify(r))
};
var l=g.prototype;
var j=p.instance;
return{setItem:l.setItem.bind(j,n,o),getItem:l.getItem.bind(j,o),removeItem:l.removeItem.bind(j,n,o),getKeys:function(){return ContextHub.Utils.JSON.tree.getKeys(o())
},getTree:o}
},function a(){var k="contexthub.test."+this.name;
var j;
try{var m=f.localStorage;
m.setItem(k,k);
j=m.getItem(k)===k;
m.removeItem(k)
}catch(l){j=false
}return j
}),SESSION:new g("session",function d(p){var m=p.config;
var k=m.container;
var q=m.window.sessionStorage;
var o=function(){var r=ContextHub.Utils.JSON.parse(q.getItem(k));
return(e.type(r)==="object")?r:{}
};
var n=function(r){q.setItem(k,ContextHub.Utils.JSON.stringify(r))
...
var j;
try{var m=f.sessionStorage;
m.setItem(k,k);
j=m.getItem(k)===k;
m.removeItem(k)
}catch(l){j=false
}return j
}),COOKIE:new g("cookie",function d(p){var m=p.config;
var k=m.container;
var q=ContextHub.Utils.Cookie;
var o=function(){var r=ContextHub.Utils.JSON.parse(q.getItem(k));
return(e.type(r)==="object")?r:{}
};
var n=function(r){q.setItem(k,ContextHub.Utils.JSON.stringify(r))
...
h.cookie=k+"=1";
var j=h.cookie.indexOf(k)!==-1;
h.cookie=k+"=; expires=Thu, 01-Jan-1970 00:00:01 GMT";
return j
}),WINDOW:new g("window",function d(p){var m=p.config;
var k=m.container;
var q=m.window;
var o=function(){var r=ContextHub.Utils.JSON.parse(q.name)[k];
return(e.type(r)==="object")?r:{}
};
var n=function(r){var s=ContextHub.Utils.JSON.parse(q.name);
s[k]=r;
q.name=ContextHub.Utils.JSON.stringify(s)
};
var l=g.prototype;
var j=p.instance;
return{setItem:l.setItem.bind(j,n,o),getItem:l.getItem.bind(j,o),removeItem:l.removeItem.bind(j,n,o),getKeys:function(){return ContextHub.Utils.JSON.tree.getKeys(o())
...
ContextHub.console("log","[+] loading [contexthub.utils] ContextHub.Utils.storeCandidates.js");
(function(e,b){b.ContextHub=e.extend({Utils:{}},b.ContextHub);
var d={};
var c=function(h,j,i,g){var k={store:h,priority:i,applies:g};
if(j in d){var f=d[j];
f.push(k);
f.sort(function(m,l){return l.priority-m.priority
})
}else{d[j]=[k]
}};
var a=function(j){if(j in d){var f=d[j];
if(f.length){for(var g=0;
g<f.length;
g++){var h=f[g];
if(h.applies()){return h.store
}}}}throw new Error("No suitable implementation found for store '"+j+"'")
};
ContextHub.Utils.storeCandidates={registerStoreCandidate:c,getStoreFromCandidates:a}
}(ContextHubJQ,this));
ContextHub.console("log","[+] loading [contexthub.utils] ContextHub.Utils.inheritance.js");
(function(b,a){a.ContextHub=b.extend({Utils:{}},a.ContextHub);
a.ContextHub.Utils.inheritance={inherit:function(g,c){g.prototype=new c();
var f={},e=g.prototype;
g.prototype.constructor=c;
g.prototype.uber=function(h){if(!(h in f)){f[h]=0
}var k,j,i=f[h],d=c.prototype;
if(i){while(i){d=d.constructor.prototype;
i-=1
}k=d[h]
}else{k=e[h];
if(k==this[h]){k=d[h]
}}f[h]+=1;
j=k.apply(this,Array.prototype.slice.apply(arguments,[1]));
f[h]-=1;
return j
};
return g
...
ContextHub.console("log","[+] loading [contexthub.kernel] ContextHub.js");
(function(e,h,n,f){if(typeof(ContextHubKernelConfig)==="undefined"){ContextHub.console("log","[-] ContextHub configuration is not set!")
}h.ContextHub=e.extend({},h.ContextHub);
var d={};
var m=[];
var g=[];
var t=new ContextHub.Utils.Persistence();
var o=new ContextHub.Utils.Eventing(ContextHubKernelConfig.eventing);
var c=function(w,v){if(e.type(v)==="object"){d=e.extend({},d);
if(!d[w]){d[w]=v;
ContextHub.console("timeStamp",'registering "'+w+'"');
this.eventing.trigger(ContextHub.Constants.EVENT_REGISTER+":"+w,{},{defer:0,_:{action:"register",store:w,registeredAt:+new Date}});
if(!v.queryService){v.announceReadiness()
}}}};
var k=function(){return d
};
var q=function(v){return((e.type(v)==="string")&&v.length)?ContextHub.Utils.JSON.tree.getItem(d,v):null
};
var i=function(v){return this.persistence.getItem(v)
};
var b=function(w,x){var y=e.trim("/"+(w||"")).replace(/\/+/g,"/").match(/^\/(store)\/([^/]+)\/(.*)/)||[];
var z=ContextHub.persistence;
y.shift();
if(y&&y.shift()==="store"){var v=ContextHub.getStore(y.shift());
if(v){z=v;
w=y.shift()
}}z.setItem(w,x)
};
var s=function(v){this.persistence.removeItem(v)
};
var l=function(w){var v={};
var x=0;
e.each(w,function(y,z){if(z){var B=ContextHub.Utils.JSON.tree.sanitizeKey(z);
if(B[0]==="store"){z="/"+B.slice(1).join("/");
B=z
}else{B=e.merge(["store"],B)
}B="/"+B.join("/");
var A=ContextHub.getItem(B);
if(A){x++;
v[z]=A
}}});
v._length=x;
...
var j=function(z,x,y,C){if(e.type(z)==="string"){z=[z]
}x=x||function(){};
y=y||function(){};
var w=l(z);
if(w._length==z.length){x(w);
return
}var B=0;
var A=false;
var v=function(E){if(A){return true
}var D=l(z);
var F=null;
if(e.type(E)==="undefined"){F=y
}if(D._length==z.length){F=x;
clearTimeout(B)
}if(F){A=true;
F(D)
}return A
...
var r=function(){var v=o.isPaused();
this.eventing.pause();
e.each(k(),function(w,x){x.clean()
});
if(!v){this.eventing.resume()
}};
var p=function(w){var v=o.isPaused();
this.eventing.pause();
e.each(k(),function(x,y){y.reset(w)
});
if(!v){this.eventing.resume()
}};
var u=function(z,x,y,B){if(e.type(z)==="string"){z=[z]
}x=x||function(){};
y=y||function(){};
var w=l(z);
var C=true;
if(w._length==z.length){C=false;
x(w)
}var A=0;
var v=function(G,E){if(E){var D=true;
e.each(z,function(J,I){if(E[I]){D=false
}return D
});
if(D){return
}}var F=l(z);
var H=y;
if(F._length==z.length){H=x;
clearTimeout(A)
}H(F)
};
if(C){A=setTimeout(v,B)
}g.push(v)
};
e.extend(ContextHub,{persistence:t,eventing:o,registerStore:c,getAllStores:k,getStore:q,getItem:i,setItem:b,removeItem:s,cleanAllStores:r,resetAllStores:p,sync:j,bind:u});
ContextHub.eventing.on(ContextHub.Constants.EVENT_DATA_UPDATE,function(w,x){e.each(m||[],function(z,y){if(y){if(y(z)){m[z]=f;
m=e.grep(m,e.isFunction)
}}});
var v={};
e.each(x.data||[],function(z,y){v["/"+x.store+"/"+y.key]=true
});
e.each(g||[],function(z,y){if(y){y(z,v)
}})
},"sync-bind");
var a={};
ContextHub.eventing.on(ContextHub.Constants.EVENT_STORE_READY,function(w,x){a[x.store]=true;
var v=true;
e.each(ContextHubKernelConfig.stores,function(y,z){if(!a[y]&&z.required){v=false
}});
if(v){ContextHub.console("timeStamp","contexthub initialized");
ContextHub.eventing.off(w.type);
ContextHub.eventing.trigger(ContextHub.Constants.EVENT_INITIALIZED,{},{defer:0,_:{action:"initialized",wasReadyAt:+new Date}})
}})
}(ContextHubJQ,this,document));
ContextHub.console("log","[+] loading [contexthub.generic-stores] ContextHub.Store.Core.js");
(function(e,d,a,f){d.ContextHub=e.extend({Store:{}},d.ContextHub);
var c=new ContextHub.Utils.Persistence.prototype.PersistenceMode("memory",function b(m){var j=m.config;
var h=j.container;
var n=j.storage;
var l=function(){var o=((e.type(n)==="object")?n:{})[h];
return(e.type(o)==="object")?o:{}
};
var k=function(o){n=(e.type(n)==="object")?n:{};
n[h]=o
};
var i=ContextHub.Utils.Persistence.prototype.PersistenceMode.prototype;
var g=m.instance;
return{setItem:i.setItem.bind(g,k,l),getItem:i.getItem.bind(g,l),removeItem:i.removeItem.bind(g,k,l),getKeys:function(){return ContextHub.Utils.JSON.tree.getKeys(l())
},getTree:l}
});
ContextHub.Store.Core=function(h,g){};
ContextHub.Store.Core.prototype.init=function(h,g){this.config=e.extend({},ContextHub.Store.Core.defaultConfig,g);
this.name=h;
this.eventChannel=ContextHub.Constants.EVENT_DATA_UPDATE+":"+this.name;
this.storeDataKey="/store/"+this.name;
this.data={};
this.references={};
if(!this.config.persistence){this.config.persistence=new ContextHub.Utils.Persistence({container:"data",mode:c,storage:this.data})
}if(!this.config.eventing){this.config.eventing={trigger:function(j,k,i){},isPaused:function(i){},pause:function(i){},resume:function(i){}}
...
ContextHub.Store.Core.prototype.reset=function(h){var g=this.isEventingPaused();
this.pauseEventing();
if(!h){this.clean()
}var i=this.config.initialValues||{};
this.addAllItems(i);
if(!g){this.resumeEventing()
}};
ContextHub.Store.Core.prototype.setItem=function(m,l,n){m=this.resolveReference(m);
var o=this;
var g=this.getItem(m);
var i=true;
var h=typeof(l);
if(typeof(g)===h){if((h==="string"||h==="number"||h==="boolean")&&(g==l)){i=false
}else{var k=ContextHub.Utils.JSON.stringify(g);
var j=ContextHub.Utils.JSON.stringify(l);
i=(k.length!==j.length)||(k!==j)
}}if(i){o=this.persistence.setItem(this.storeDataKey+"/"+m,l);
if(!n||!n.silent){this.eventing.trigger(this.eventChannel,{key:m,value:l,old:g,action:"set"},e.extend({defer:this.config.eventDeferring,_:{store:this.name}},n))
}}return o
};
ContextHub.Store.Core.prototype.getItem=function(g){g=this.resolveReference(g);
return this.persistence.getItem(this.storeDataKey+"/"+g)
};
ContextHub.Store.Core.prototype.removeItem=function(j,i){j=this.resolveReference(j);
var g=this;
var h=this.getItem(j);
if(h!==null){g=this.persistence.removeItem(this.storeDataKey+"/"+j);
if(!i||!i.silent){this.eventing.trigger(this.eventChannel,{key:j,value:null,old:h,store:this.name,action:"remove"},e.extend({defer:this.config.eventDeferring},i))
}}return g
};
ContextHub.Store.Core.prototype.getKeys=function(h){var g=this.persistence.getTree();
g=ContextHub.Utils.JSON.tree.getItem(g,this.storeDataKey)||{};
if(!h){delete (g._)
}return ContextHub.Utils.JSON.tree.getKeys(g)
};
ContextHub.Store.Core.prototype.getTree=function(h){var g=this.persistence.getItem(this.storeDataKey)||{};
if(!h){delete (g._)
}return g
};
ContextHub.Store.Core.prototype.addAllItems=function(g,h){var i=e.type(g);
if(i==="object"||i==="array"){e.each(g,function(j,k){this.setItem(j,k,h)
}.bind(this))
}return this
};
ContextHub.Store.Core.prototype.addReference=function(h,g){h=ContextHub.Utils.JSON.tree.sanitizeKey(h);
g=ContextHub.Utils.JSON.tree.sanitizeKey(g);
if(h&&g){h="/"+h.join("/");
g="/"+g.join("/");
if(h!==g){this.references[h]=g
}}};
ContextHub.Store.Core.prototype.removeReference=function(g){g=ContextHub.Utils.JSON.tree.sanitizeKey(g);
if(g){g="/"+g.join("/");
delete (this.references[g])
}};
ContextHub.Store.Core.prototype.getReferences=function(){return this.references
};
ContextHub.Store.Core.prototype.resolveReference=function(j,i){if(e.isEmptyObject(this.references)){return j
}j=ContextHub.Utils.JSON.tree.sanitizeKey("/"+j);
var m=j;
if(j){m="/"+j.join("/");
i=i||5;
var h=true;
while(i-->0&&h){var n=ContextHub.Utils.JSON.tree.sanitizeKey(m);
var l="";
var k=n.slice(0);
for(var g=0;
g<n.length&&!this.references[l];
g++){l+="/"+n[g];
k.shift()
}h=this.references[l];
if(h){h=h+"/"+k.join("/");
h="/"+ContextHub.Utils.JSON.tree.sanitizeKey(h).join("/")
}m=h||m
}}return m
};
...
ContextHub.Store.Core.prototype.announceReadiness=function(){var g=this.name;
ContextHub.console("timeStamp",'"'+g+'" ready');
this.eventing.trigger(ContextHub.Constants.EVENT_STORE_READY+":"+g,{},{defer:0,_:{action:"ready",store:g,wasReadyAt:+new Date,duration:this.duration||0}})
}
}(ContextHubJQ,this,document));
ContextHub.console("log","[+] loading [contexthub.generic-stores] ContextHub.Store.SessionStore.js");
(function(c,b,a,d){b.ContextHub=c.extend({Store:{}},b.ContextHub);
...
ContextHub.Store.SessionStore.prototype.init=function(f,e){this.uber("init",f,e);
this.config=c.extend({},this.config,ContextHub.Store.SessionStore.defaultConfig,e)
}
}(ContextHubJQ,this,document));
ContextHub.console("log","[+] loading [contexthub.generic-stores] ContextHub.Store.PersistedStore.js");
(function(c,b,a,d){b.ContextHub=c.extend({Store:{}},b.ContextHub);
...
ContextHub.Store.PersistedStore.prototype.init=function(f,e){this.uber("init",f,e);
this.config=c.extend({},this.config,ContextHub.Store.PersistedStore.defaultConfig,e)
}
}(ContextHubJQ,this,document));
ContextHub.console("log","[+] loading [contexthub.generic-stores] ContextHub.Store.JSONPStore.js");
(function(b,a){a.ContextHub=b.extend({Store:{}},a.ContextHub);
ContextHub.Store.JSONPStore=function(d,c){};
...
this.config=b.extend({},this.config,ContextHub.Store.JSONPStore.defaultConfig,c)
};
...
ContextHub.Store.JSONPStore.prototype.resolveParameter=function(e){e=(b.type(e)==="boolean")?String(e):e;
var c=e||"";
var d=c.match(/\${contexthub:[^}]+}/g);
if(d){b.each(d,function(f,h){var g=h.split(":").pop().slice(0,-1);
c=c.replace(h,ContextHub.persistence.getItem(g)||"")
})
}return c
};
ContextHub.Store.JSONPStore.prototype.getServiceURL=function(e){var c=this.getServiceDetails();
if(b.type(c)!=="object"){return null
}if(c.jsonp){c.params=b.extend({callback:"ContextHub.Callbacks."+this.name},c.params)
}var d=[];
e=e===true;
c.port=(c.port&&c.port===80)?"":c.port;
if(c.host){if(c.secure==="auto"){d.push("//")
}else{d.push(c.secure?"https://":"http://")
}d.push(c.host);
d.push(c.port?(":"+c.port):"")
}c.path=c.path||"/";
d.push((e?this.resolveParameter(c.path):c.path));
var f=[];
b.each(b.extend({},c.params),function(g,h){f.push(encodeURIComponent(g)+"="+encodeURIComponent(e?this.resolveParameter(h):h))
}.bind(this));
if(f.length){d.push("?");
d.push(f.join("&"))
}return d.join("")
};
ContextHub.Store.JSONPStore.prototype.queryService=function(h){var f=this.getServiceURL(true);
if(!f){return
}if(h){this.removeItem("_",{silent:true})
}var g=this.getItem(ContextHub.Constants.SERVICE_RESPONSE_TIME_KEY)||0;
var c=this.getItem(ContextHub.Constants.SERVICE_LAST_URL_KEY);
if((g+this.config.service.ttl>+new Date)&&(c==f)){this.duration="cached";
this.announceReadiness();
return
}this.setItem(ContextHub.Constants.SERVICE_LAST_URL_KEY,f);
a.ContextHub=b.extend({Callbacks:{}},a.ContextHub);
ContextHub.Callbacks[this.name]=this.callbackFunction.bind(this);
var i=this.config.service.timeout;
var e=this;
var d=e.isEventingPaused();
e.pauseEventing();
this.duration=0;
ContextHub.Shared.timers.start(this.name);
if(this.config.service.jsonp){b.ajax({url:f,timeout:i,dataType:"script"}).fail(function(j){e.failureHandler(j)
}).always(function(){if(!d){e.resumeEventing()
}e.announceReadiness()
})
}else{b.ajax({url:f,timeout:i}).done(function(j,k,l){ContextHub.Callbacks[e.name](ContextHub.Utils.JSON.parse(l.responseText))
}).fail(function(j){e.failureHandler(j)
}).always(function(){if(!d){e.resumeEventing()
}e.announceReadiness()
})
}};
...
this.setItem(ContextHub.Constants.SERVICE_RESPONSE_TIME_KEY,+new Date);
...
ContextHub.console("log","[+] loading [contexthub.generic-stores] ContextHub.Store.PersistedJSONPStore.js");
(function(c,b,a,d){b.ContextHub=c.extend({Store:{}},b.ContextHub);
...
ContextHub.Store.PersistedJSONPStore.prototype.init=function(f,e){this.uber("init",f,e);
this.config=c.extend({},this.config,ContextHub.Store.PersistedJSONPStore.defaultConfig,e)
}
}(ContextHubJQ,this,document));
ContextHub.console("log","[+] loading [contexthub.store.contexthub.geolocation] store.geolocation.js");
(function(c){var a={service:{jsonp:false,timeout:1000,ttl:30*60*1000,secure:false,host:"maps.googleapis.com",port:80,path:"/maps/api/geocode/json"},eventDeferring:16,html5coordinatesDiscoveryAPI:{timeout:30000,ttl:15*60*1000,highAccuracy:false},initialValues:{latitude:37.331375,longitude:-121.893992}};
var b=function(e,d){this.config=c.extend({},a,d);
c.extend(this.config.service,{params:{sensor:false,latlng:"${contexthub:/store/"+e+"/latitude},${contexthub:/store/"+e+"/longitude}"}});
this.init(e,this.config);
var f=this.config.initialValues;
c.extend(this.config.html5coordinatesDiscoveryAPI,{location:f});
this.queryService(false,true)
};
ContextHub.Utils.inheritance.inherit(b,ContextHub.Store.PersistedJSONPStore);
c.extend(b.prototype,{getCoordinates:function(){var d=c.Deferred();
if(!navigator.geolocation){return d.reject()
}navigator.geolocation.getCurrentPosition(d.resolve,d.reject,{maximumAge:this.config.html5coordinatesDiscoveryAPI.ttl,timeout:this.config.html5coordinatesDiscoveryAPI.timeout,enableHighAccuracy:this.config.html5coordinatesDiscoveryAPI.highAccuracy});
return d.promise()
},loadCurrentLocation:function(){var d=this;
c.when(this.getCoordinates()).then(function(e){d.loadLocation(e.coords||{})
})
},loadLocation:function(d){d=d||{};
if(c.isNumeric(d.latitude)&&c.isNumeric(d.longitude)){this.storeLocation(d,{});
this.queryService(false,true)
}},successHandler:function(i){if(i&&i.status==="OK"){var g=null;
var l=null;
var j=null;
var f=null;
var h=null;
...
var e=null;
var k=i.results||[];
c.each(k.reverse(),function(n,o){o=o||{};
e=o.formatted_address||e;
c.each(o.address_components||[],function(p,r){var q=r.types||[];
if(c.inArray("administrative_area_level_1",q)!==-1){m=r.short_name||m
}if(c.inArray("country",q)!==-1){g=r.long_name||g;
l=r.short_name||l
}if(c.inArray("locality",q)!==-1){j=r.short_name||j
}if(c.inArray("route",q)!==-1){f=r.short_name||f
}if(c.inArray("street_number",q)!==-1){h=r.short_name||h
}if(c.inArray("postal_code",q)!==-1){d=r.short_name||d
}})
});
this.setItem("address",{country:g||"",countryCode:l||"",city:j||"",street:f||"",streetNumber:h||"",region:m||"",name:e||"",postalCode:d||""})
}return i
},failureHandler:function(d){ContextHub.console("log","Error while getting geolocation information:",d)
},storeLocation:function(d,e){d=c.extend(e,d);
this.setItem("latitude",parseInt(d.latitude*1000000)/1000000);
this.setItem("longitude",parseInt(d.longitude*1000000)/1000000)
},areCoordinatesSet:function(){return c.isNumeric(this.getItem("latitude"))&&c.isNumeric(this.getItem("longitude"))
},queryService:function(e,f){var d=this;
if(f&&!this.areCoordinatesSet()){f=false
}if(f){this.uber("queryService",e)
}else{c.when(this.getCoordinates()).then(function(g){d.storeLocation(g.coords||{},d.config.html5coordinatesDiscoveryAPI.location);
d.uber("queryService",e)
},function(g){d.storeLocation({},d.config.html5coordinatesDiscoveryAPI.location);
d.uber("queryService",e)
})
}}});
ContextHub.Utils.storeCandidates.registerStoreCandidate(b,"contexthub.geolocation",0,function(){return"geolocation" in navigator
})
}(ContextHubJQ,this,document));
ContextHub.console("log","[+] loading [cq.contexthub] ContextHub.Store.ClientContextBackedStore.js");
(function(a){window.ContextHub=a.extend({Store:{}},window.ContextHub);
...
this.config=a.extend({},this.config,ContextHub.Store.ClientContextBackedStore.defaultConfig,b)
};
ContextHub.Store.ClientContextBackedStore.defaultConfig={forwardToClientContext:true};
ContextHub.Store.ClientContextBackedStore.prototype.successHandler=function(c){this.config.mappingConfig.forwardToClientContext=false;
var e,g;
var b=this.isEventingPaused();
this.pauseEventing();
this.setItem("/",{});
for(e in this.config.mappingConfig.propertyMapping){g=this.config.mappingConfig.propertyMapping[e];
this.setItem("/"+e,ClientContext.get(this.config.mappingConfig.clientContextStoreName+"/"+g))
}if(this.config.mappingConfig.mapOtherProperties){var f=ClientContext.get(this.config.mappingConfig.clientContextStoreName).data;
var d=a.map(this.config.mappingConfig.propertyMapping,function(h){return h
});
if(f){for(g in f){if(a.inArray(g,d)===-1){this.setItem("/"+g,f[g])
}}}}if(!b){this.resumeEventing()
}this.config.mappingConfig.forwardToClientContext=true
};
ContextHub.Store.ClientContextBackedStore.prototype.queryService=function(e){var c=this.isEventingPaused();
this.pauseEventing();
var b=ClientContext.get(this.config.mappingConfig.clientContextStoreName);
if(b){var d=this;
b.addListener("update",function(){d.successHandler()
});
this.successHandler()
}else{this.failureHandler()
}if(!c){this.resumeEventing()
}this.announceReadiness()
};
ContextHub.Store.ClientContextBackedStore.prototype.setItem=function(c,d){this.uber("setItem",c,d);
if(this.config.mappingConfig.forwardToClientContext){if(c[0]=="/"){c=c.substring(1)
}var b=this.config.mappingConfig.propertyMapping[c];
if(typeof b==="undefined"){if(this.config.mappingConfig.mapOtherProperties){ClientContext.set(this.config.mappingConfig.clientContextStoreName+"/"+c,d)
}else{ContextHub.console("warn","No mapping found for CH property '"+c+"' ; not forwarding.")
}}else{ClientContext.set(this.config.mappingConfig.clientContextStoreName+"/"+b,d)
}}}
}(ContextHubJQ));
(function(b){window.ContextHub=b.extend({Helpers:{}},window.ContextHub);
var a={mappingConfig:{clientContextStoreName:"geolocation",propertyMapping:{},mapOtherProperties:true}};
var c=function(e,d){this.config=b.extend({},a,d);
this.init(e,this.config);
this.queryService()
};
ContextHub.Utils.inheritance.inherit(c,ContextHub.Store.ClientContextBackedStore);
c.prototype.failureHandler=function(d){};
...
ContextHub.Utils.storeCandidates.registerStoreCandidate(c,"contexthub.geolocation",10,function(){return !!window.ClientContext
})
}(ContextHubJQ));
ContextHub.console("log","[+] loading [contexthub.store.contexthub.surferinfo] Store.surferinfo.js");
(function(d,g){var e;
...
var k=["Opera","Chromium","Chrome","Safari","Trident","MSIE","Firefox","AppleWebKit"];
...
var l={"NT 6.3":"8.1","NT 6.2":"8","NT 6.1":"7","NT 6.0":"Vista","NT 5.2":"XP x64","NT 5.1":"XP","NT 5.01":"2000 SP1","NT 5.0":"2000","NT 4.0":"NT 4.0","98":"98","NT 95":"95","NT CE":"CE"};
k=l[o];
...
var m=g.screen.colorDepth;
var o=h("display",{resolution:{width:g.screen.width*k,height:g.screen.height*k},devicePixelRatio:k,colorDepth:m,nrOfColors:Math.pow(2,m),pixelsPerInch:f(),orientation:b(g)}).display.resolution
}if(!this.getItem("window")){var l=h("window",{dimension:{width:d(g).innerWidth()*k,height:d(g).innerHeight()*k}}).window.dimension;
h("window",{percentageUsage:Math.floor(((l.width*l.height)/(o.width*o.height))*100)/100})
}if(!this.getItem("browser")){h("browser",j())
}var n=c();
if(!this.getItem("device")){h("device",n)
}if(!this.getItem("isMobile")){h("isMobile",n.type!=="desktop")
...
ContextHub.Utils.storeCandidates.registerStoreCandidate(i,"contexthub.surferinfo",0,function(){return true
})
}(ContextHubJQ,this));
ContextHub.console("log","[+] loading [contexthub.store.granite.profile] Store.profile.js");
(function(c){var a={service:{jsonp:false,timeout:1000,path:"${contexthub:/store/profile/path}.infinity.json"},initialValues:{path:"/home/users/a/anonymous"}};
var b=function(e,d){this.config=c.extend({},a,d);
this.init(e,this.config);
var f=this.getItem("path");
if(!f){this.setItem("path",this.config.initialValues.path)
}this.queryService(false)
};
ContextHub.Utils.inheritance.inherit(b,ContextHub.Store.PersistedJSONPStore);
b.prototype.successHandler=function(f){this.pauseEventing();
var h=this.getItem("path");
var g=/\/(jcr|sling|):/;
var i=f.profile;
var d=c.extend(true,{},i);
c.each(ContextHub.Utils.JSON.tree.getKeys(d),function(j,k){if(k.match(g)){d=ContextHub.Utils.JSON.tree.removeItem(d,k)
}});
d=ContextHub.Utils.JSON.tree.setItem(d,"path",h);
var e="anonymous";
if(i.givenName&&i.familyName){e=i.givenName+" "+i.familyName
}else{if(i.givenName){e=i.givenName
}else{if(i.familyName){e=i.familyName
}}}d=ContextHub.Utils.JSON.tree.setItem(d,"displayName",e);
if(i.photos&&i.photos.primary&&i.photos.primary.image){d=ContextHub.Utils.JSON.tree.setItem(d,"avatar",h+"/profile/photos/primary/image")
}this.setItem("/",d);
this.resumeEventing()
};
b.prototype.loadProfile=function(e){var d=ContextHub.getStore("profile");
d.setItem("path",e);
d.queryService(true)
};
ContextHub.Utils.storeCandidates.registerStoreCandidate(b,"granite.profile",0,function(){return true
})
}(ContextHubJQ));
ContextHub.console("log","[+] loading [contexthub.profile.kernel] store.clientcontextprofile.js");
(function(b){window.ContextHub=b.extend({Helpers:{}},window.ContextHub);
var a={mappingConfig:{clientContextStoreName:"profile",propertyMapping:{displayName:"formattedName",path:"path",avatar:"avatar"},mapOtherProperties:true}};
var c=function(e,d){this.config=b.extend({},a,d);
this.init(e,this.config);
this.queryService()
};
ContextHub.Utils.inheritance.inherit(c,ContextHub.Store.ClientContextBackedStore);
c.prototype.loadProfile=function(e){var d=e.split("/");
if(d.length){var f=d[d.length-1];
ClientContext.get(this.config.mappingConfig.clientContextStoreName).loadProfile(f)
}};
c.prototype.reset=function(){ClientContext.get(this.config.mappingConfig.clientContextStoreName).reset()
};
c.prototype.failureHandler=function(d){};
ContextHub.Utils.storeCandidates.registerStoreCandidate(c,"granite.profile",10,function(){return !!window.ClientContext
})
}(ContextHubJQ));
ContextHub.console("log","[+] loading [contexthub.finalize] ContextHub.finalization.js");
(function(a){if(ContextHubKernelConfig&&ContextHubKernelConfig.stores){ContextHub.Shared.timers.start("contexthub-config");
a.each(ContextHubKernelConfig.stores,function(d,g){var b=ContextHub.Utils.storeCandidates.getStoreFromCandidates(g.type);
if(b){try{var c=new b(d,g.config);
if(c){ContextHub.registerStore(d,c)
}}catch(f){ContextHub.console("error","Error while instantiating store for "+g.type,b,f)
}}});
ContextHub.eventing.trigger(ContextHub.Constants.EVENT_CONFIG_LOADED,{},{defer:0,_:{action:"config-loaded",wasReadyAt:+new Date,duration:ContextHub.Shared.timers.finish("contexthub-config")+"ms"}})
}}(ContextHubJQ));
